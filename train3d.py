...
TEST_SIZE = 0.2
LOSS_FUNC = 'Dice'
OPTIMIZER = 'SGD_lr_0p001'
FOLDER = '/rds/general/user/rsb17/home/FYP/Tests3d/test_'+str(UNIQUEID)+'_EPO_'+str(EPOCHS)+'_Testsize_'+str(TEST_SIZE)+'_Loss_'+str(LOSS_FUNC)+'_Opt_'+str(OPTIMIZER)+'/'
Model_Name = FOLDER+'model.hdf5'
#os.mkdir(os.path.abspath(os.getcwd())+FOLDER,mode = 0o666)
...
def get_model(image_dataset,EPOCHS,X_train,y_train,X_test,y_test,Model_Name,FOLDER):
    ...
    path='/rds/general/user/rsb17/home/FYP/data/evaluate/image.mhd'
    ...
    prediction_other = (model.predict(test_img_other_input)[0,:,:,:,0] > 0.9).astype(np.uint8)
